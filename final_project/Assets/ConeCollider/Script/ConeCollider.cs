...
public class ConeCollider : MonoBehaviour {
    ...
    //Constuctor for ConeCollider
    public ConeCollider(float newDistance, float newAngle)
    {
    ...
    void Awake()
    {
        ...
        var centerForwardPos = this.transform.position + forward * distance;
        ...
        for (int i = 0; i < vertices.Length; i++) {
            ...
            if (verticleCount != 2 || i >= 36) {
                //既に距離1分あるので-1
                verticeWorldPos += forward * (distance - 1);
    ...
    public void updateProperties(float newDistance, float newAngle)
    {
        Debug.Log("Updating ColliderCone");
        angle = newAngle;
        distance = newDistance;
    }
...
public class ConeColliderEditor : Editor {
    ...
    void OnSceneGUI() {
        if (!EditorApplication.isPlaying) {
            ...
            var centerForward = m_conecollider.transform.position + m_conecollider.transform.TransformDirection(Vector3.forward) * distance.floatValue;
